...
<form role="form" method="post" action="VARIABLE_OPEN_PHP echo site_url("<?php echo ($title); ?>/addPost"); VARIABLE_CLOSE_PHP" enctype="multipart/form-data">
<input type="hidden" name="VARIABLE_OPEN_PHP echo $this->config->item('csrf_token_name'); VARIABLE_CLOSE_PHP" value="VARIABLE_OPEN_PHP echo $this->security->get_csrf_hash(); VARIABLE_CLOSE_PHP">
 ...
 <?php foreach($items as $item){ ?>
  <div class="form-group">
    ...
    <input type="text" value="" class="form-control" id="<?php echo ($title."_".$item); ?>" name="<?php echo ($title."_".$item); ?>">
  </div>
  <?php } ?>
